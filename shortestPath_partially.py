...
def constructGraph(readList, gapStart, gapEnd, template, fill, startRead, endRead):
	...
	if fill == "left" or startRead[1] < gapStart:
	...
	for i in range(len(readList)):
		...
		for j in range(i+1,len(readList)):
			...
			secondSuffixStart = firstEnd - gapStart -1
			secondSuffixEnd = secondEnd - gapStart -1
...
# 				else:
	...
	return (graph,readList[0][0],readList[-1][0], readList[-1][3], readList[0][3])
...
def reconstructSequence2(path,graph, startSeq):
	sequence = startSeq
	for i in range(len(path)-1):
		node = path[i]
		next = path[i+1]
